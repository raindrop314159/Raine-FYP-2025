...
% Specify the date for graph plotting
dateInput = ['20240903'];
...
% Retrieve AQMS data
data_path ='AQMS.csv';
[AQMS_hourly_exposure, AQMS_PM25_in_hour, AQMS_PM10_in_hour, AQMS_O3_in_hour, AQMS_NO2_in_hour] = f_PEK(num_dateInput, data_path);

disp('Finished conversion of AQMS')
...
%Initialize Praise Outdoor variables
...
% Total exposure
Total_exposure = 0 ;
Total_outdoor_exposure = 0 ;
Total_AQMS_exposure = 0;

conc_PM2_5 = 0;
conc_SO2 = 0;

for i = 1:height(Praise)
    ...
    if Praise{i,'exposure_parameter'}{1} == "Bus and Minibus"
        IO = 8;
    elseif Praise{i,'exposure_parameter'}{1} == "MTR"
        IO = 7;
    elseif Praise{i,'exposure_parameter'}{1} == "Outdoor"
        IO = 6;
    elseif Praise{i,'exposure_parameter'}{1} == "Vehicle"
        IO = 5;
    elseif Praise{i,'exposure_parameter'}{1} == "Other Indoor"
        IO = 4;
    elseif Praise{i,'exposure_parameter'}{1} == "Office"
        IO = 3; 
    elseif Praise{i,'exposure_parameter'}{1} == "School"
        IO = 2;
    elseif Praise{i,'exposure_parameter'}{1} == "Home"
        IO = 1;
    end
    ...
    time1 = Praise.startTime(i); % First time
    time2 = Praise.endTime(i); % Second time
    
    % Convert the numeric values to datetime
    dt1 = datetime(num2str(time1), 'InputFormat', 'yyyyMMddHHmm'); 
    dt2 = datetime(num2str(time2), 'InputFormat', 'yyyyMMddHHmm');
